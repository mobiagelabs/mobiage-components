...
class MbgInputIeController {
    ...
    $onInit() {
        this.$timeout(() => {
            console.log('aydgaysd', this.validateStateRegistration('9039419286', 'AC'))
        })
    }
    ...
    strzero(dado, tam) {
        var varTemp = this.removeIncorrectCharacter(dado);
        var varTemp = dado;
    ...
    validateStateRegistration(theField, estado) {
        ...
        let somaAux
        let mult
        if (theField.toLowerCase() === 'isento') {
            return
        }
        theField = this.removeIncorrectCharacter(theField)
        if (inscEst != "") {
            var dig8 = "/BA*/RJ*";
